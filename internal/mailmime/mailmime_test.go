...
func TestBuildRoundTrip(t *testing.T) {
	...
	raw, err := mailmime.Build(&mailmime.Message{
		MessageID: "<abc@polygone.club>",
		From:      "Lucas <hello@polygone.club>",
	...
	if idx.MessageID != "<abc@polygone.club>" {
		t.Errorf("message-id = %q", idx.MessageID)
	}
	if !strings.Contains(idx.From, "hello@polygone.club") {
...
const appleMailSubmission = "From: Lucas <hello@polygone.club>\r\n" +
	"To: Friend <friend@example.test>\r\n" +
	"Cc: cc@example.test\r\n" +
	"Subject: Re: hello\r\n" +
	"Message-Id: <new@polygone.club>\r\n" +
...
func TestParseSubmission(t *testing.T) {
	...
	if sub.From != `"Lucas" <hello@polygone.club>` && sub.From != "Lucas <hello@polygone.club>" {
...
func TestAddressHelpers(t *testing.T) {
	cases := []struct{ in, addr, domain string }{
		{"Lucas <hello@polygone.club>", "hello@polygone.club", "polygone.club"},
