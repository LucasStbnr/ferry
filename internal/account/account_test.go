...
func newFixture(t *testing.T) *fixture {
	...
	api.SetDomains(
		resend.Domain{ID: "d1", Name: "polygone.club", Status: "verified",
...
func TestAddCreatesEverything(t *testing.T) {
	...
	created, err := f.mgr.Add(ctx, account.AddOptions{Name: "polygone", APIKey: apiKey})
	if err != nil {
		t.Fatalf("add: %v", err)
	}

	// Only verified domains may be used for sending.
	if len(created.Domains) != 1 || created.Domains[0] != "polygone.club" {
		t.Fatalf("domains = %v, want only the verified one", created.Domains)
	}
	if created.Account.Address != "hello@polygone.club" {
		t.Errorf("address = %q", created.Account.Address)
	}

	// The key lives in the secret store, never in the database.
	stored, err := f.sec.Get(secrets.APIKey("polygone"))
	if err != nil || stored != apiKey {
		t.Fatalf("api key = %q, %v", stored, err)
	}
	acct, err := f.db.AccountByName(ctx, "polygone")
	if err != nil {
		t.Fatal(err)
	}
...
func TestAddRejectsABadKeyWithoutLeavingState(t *testing.T) {
	...
	if _, err := f.mgr.Add(ctx, account.AddOptions{Name: "polygone", APIKey: "re_wrong"}); err == nil {
		t.Fatal("an invalid API key was accepted")
	}
	if _, err := f.db.AccountByName(ctx, "polygone"); !errors.Is(err, store.ErrNotFound) {
		t.Fatal("a failed add left an account row behind")
	}
	if _, err := f.sec.Get(secrets.APIKey("polygone")); !errors.Is(err, secrets.ErrNotFound) {
...
func TestAddRejectsDuplicateAndInvalidNames(t *testing.T) {
	...
	if _, err := f.mgr.Add(ctx, account.AddOptions{Name: "polygone", APIKey: apiKey}); err != nil {
		t.Fatal(err)
	}
	if _, err := f.mgr.Add(ctx, account.AddOptions{Name: "polygone", APIKey: apiKey}); !errors.Is(err, store.ErrAlreadyExists) {
...
func TestRemoveClearsMailAndSecrets(t *testing.T) {
	...
	if err := f.sec.Set(secrets.WebhookSecret("polygone"), "whsec_x"); err != nil {
		t.Fatal(err)
	}
	...
	if err := f.mgr.Remove(ctx, "polygone"); err != nil {
		t.Fatal(err)
	}
	if _, err := f.db.AccountByName(ctx, "polygone"); !errors.Is(err, store.ErrNotFound) {
	...
	if _, err := f.sec.Get(secrets.WebhookSecret("polygone")); !errors.Is(err, secrets.ErrNotFound) {
...
func TestAuthenticate(t *testing.T) {
	...
	if _, err := f.mgr.Authenticate(ctx, "polygone", created.Password); err != nil {
		t.Fatalf("the generated password was rejected: %v", err)
	}
	if _, err := f.mgr.Authenticate(ctx, "polygone", "wrong"); !errors.Is(err, account.ErrAuthFailed) {
...
func TestAuthenticateDoesNotLeakAccountNames(t *testing.T) {
	...
	real, unknown := measure("polygone"), measure("does-not-exist")
...
func TestResetPassword(t *testing.T) {
	...
	next, err := f.mgr.ResetPassword(ctx, "polygone", "")
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := f.mgr.Authenticate(ctx, "polygone", next); err != nil {
...
func TestRefreshDomainsPicksUpNewVerification(t *testing.T) {
	...
	domains, err := f.mgr.RefreshDomains(ctx, "polygone")
	if err != nil {
		t.Fatal(err)
	}
	if len(domains) != 2 {
