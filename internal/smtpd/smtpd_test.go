...
func message(from, to, subject, body string) string {
	return "From: " + from + "\r\n" +
		"To: " + to + "\r\n" +
		"Subject: " + subject + "\r\n" +
		"Message-Id: <" + subject + "@polygone.club>\r\n" +
...
func TestSendDeliversToResendAndFilesInSent(t *testing.T) {
	f := newFixture(t)
	msg := message("Lucas <hello@polygone.club>", "friend@example.test", "hello", "hi there")

	if err := f.send(t, "hello@polygone.club", []string{"friend@example.test"}, msg); err != nil {
		t.Fatalf("send: %v", err)
	}
	...
	got := sends[0]
	if !strings.Contains(got.From, "hello@polygone.club") {
...
func TestSendRequiresAuthentication(t *testing.T) {
	f := newFixture(t)
	c := f.dial(t, false)
	err := c.SendMail("hello@polygone.club", []string{"friend@example.test"},
		strings.NewReader(message("hello@polygone.club", "friend@example.test", "x", "y")))
	if err == nil {
...
func TestAuthLoginMechanism(t *testing.T) {
	...
	if err := c.SendMail("hello@polygone.club", []string{"friend@example.test"},
		strings.NewReader(message("hello@polygone.club", "friend@example.test", "viaLogin", "body"))); err != nil {
...
func TestSendFromAnUnverifiedDomainIsRefused(t *testing.T) {
	f := newFixture(t) // only polygone.club is verified
...
func TestSpoofedHeaderFromIsRefused(t *testing.T) {
	...
	err := f.send(t, "hello@polygone.club", []string{"friend@example.test"},
...
func TestBccFromEnvelopeIsCarried(t *testing.T) {
	...
	msg := message("hello@polygone.club", "friend@example.test", "party", "you are invited")
	if err := f.send(t, "hello@polygone.club",
...
func TestThreadingHeadersSurvive(t *testing.T) {
	f := newFixture(t)
	msg := "From: hello@polygone.club\r\nTo: friend@example.test\r\n" +
		"Subject: Re: hello\r\nMessage-Id: <reply@polygone.club>\r\n" +
...
func TestSignedMessageIsRefusedLoudly(t *testing.T) {
	...
	err := f.send(t, "hello@polygone.club", []string{"friend@example.test"}, msg)
	if err == nil {
...
func TestQuotaErrorIsPermanentAndExplains(t *testing.T) {
	...
	err := f.send(t, "hello@polygone.club", []string{"friend@example.test"},
		message("hello@polygone.club", "friend@example.test", "over", "body"))
	if err == nil {
...
func TestServerErrorIsTemporary(t *testing.T) {
	...
	err := f.send(t, "hello@polygone.club", []string{"friend@example.test"},
		message("hello@polygone.club", "friend@example.test", "later", "body"))
	if err == nil {
...
func TestSendIsIdempotent(t *testing.T) {
	f := newFixture(t)
	msg := message("hello@polygone.club", "friend@example.test", "once", "body")
...
func TestTooManyRecipientsIsRefused(t *testing.T) {
	f := newFixture(t)
	c := f.dial(t, true)
	if err := c.Mail("hello@polygone.club", nil); err != nil {
		t.Fatal(err)
	}
	var lastErr error
